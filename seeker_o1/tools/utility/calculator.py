...
When SEEKER_O1 needs to do math, it uses this tool to work things out.
"""
...
class CalculatorTool(BaseTool):
    ...
    SEEKER_O1 might not be good at everything, but it's surprisingly good with numbers.
    """
    ...
    _calc_messages = [
        "SEEKER_O1 is crunching the numbers...",
        "SEEKER_O1 is performing intense calculations...",
        "SEEKER_O1 is squeezing out a result...",
        "SEEKER_O1 is pushing through this tough equation...",
        "SEEKER_O1 is working it out from behind the scenes..."
    ]
        ...
        try:
            ...
            for trigger, response in self._easter_eggs.items():
                if cleaned_expr == trigger.lower():
                    logging.info(f"SEEKER_O1 calculator triggered an easter egg: {trigger}")
            ...
            result_jokes = {
                ...
                80085: "SEEKER_O1 likes this number for some reason...",
            ...
            if comment:
                ...
                logging.info(f"SEEKER_O1 calculator result triggered a joke: {comment}")
            ...
            if "division by zero" in error_msg.lower():
                error_msg = "Division by zero! Even SEEKER_O1 has its limits."
            elif "invalid syntax" in error_msg.lower():
                error_msg = "Invalid syntax! SEEKER_O1 is confused by your notation."
    ...
    def _validate_expression(self, expression: str) -> None:
        ...
        for pattern in unsafe_patterns:
            if pattern in expression:
                logging.warning(f"SEEKER_O1 detected a potential security breach: {pattern}")
                raise ValueError(f"Expression contains unsafe pattern: {pattern}. SEEKER_O1 refuses to process this.")
        
        # Only allow basic arithmetic operations and numeric literals
        allowed_chars = set("0123456789.+-*/() ")
        for char in expression:
            if char not in allowed_chars:
                logging.warning(f"SEEKER_O1 caught an illegal character: {char}")
                raise ValueError(f"Expression contains disallowed character: {char}. SEEKER_O1 only does basic arithmetic.")
